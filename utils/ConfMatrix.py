...
class ConfMatrix():
    ...
    def _compute_update(self, old_mask, pred):
        ...
        conf = torch.bincount(self.num_classes* mask.view(mask.numel()) + pred.view(pred.numel()), minlength = (self.num_classes+1)**2)
    ...
    def getClassIoU(self, class_id=None):
        if class_id is not None:
            return self.conf_matrix[class_id,class_id].float()/(torch.sum(self.conf_matrix[:, class_id]).float() + torch.sum(self.conf_matrix[class_id,:]).float() - self.conf_matrix[class_id, class_id]).float()
        else:
            return self.conf_matrix.diag().float() / (torch.sum(self.conf_matrix, 0).float() + torch.sum(self.conf_matrix,1).float() - self.conf_matrix.diag().float())
